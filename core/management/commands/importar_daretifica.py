...
class Command(BaseCommand):
    help = 'Importa contas a pagar de arquivo legado Da Retifica (Exportar_DaRetifica_Id_3.txt)'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'\n{"="*60}')
        self.stdout.write(f'  IMPORTACAO DE CONTAS A PAGAR - DA RETIFICACAO')
        ...
        for i, linha in enumerate(linhas[1:], start=1):
            ...
            try:
                ...
                data_conta = campos[3].strip().strip('"')
                dat_ven = campos[4].strip().strip('"')
                valor_par = campos[5].strip().strip('"')
